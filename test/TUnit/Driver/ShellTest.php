...
namespace Dazzle\SSH\Test\TUnit\Driver;
...
class ShellTest extends TUnit
{
     ...
     *
     */
    public function testApiGetName_ReturnsShellName()
    {
        $driver = $this->createDriver();

        $this->assertSame(SSH2::DRIVER_SHELL, $driver->getName());
    }
    ...
    public function testApiDisconnect_ClosesEachResource()
    {
        ...
        $resource1 = $this->getMock(ShellResource::class, [], [], '', false);
        $resource1
            ->expects($this->once())
            ->method('close');

        $resource2 = $this->getMock(ShellResource::class, [], [], '', false);
        $resource1
            ->expects($this->once())
            ->method('close');
     ...
     * @param mixed
     * @return Shell|\PHPUnit_Framework_MockObject_MockObject
     */
    public function createDriver($methods = null, $constructorParams = [])
    {
        ...
        $mock = $this->getMock(Shell::class, $methods, [ $ssh2, $conn ]);
